...
// password is a custom type containing the plaintext and hashed versions of a password for a user.
...
// ValidateEmail checks that the email address is not empty and that it matches
...
func (m *UserModel) GetForToken(tokenCope, tokenPlaintext string) (*User, error) {
	...
	query := `
		...
		AND tokens.scope = $2
		AND tokens.expiry > $3`
	args := []any{tokenHash[:], tokenCope, time.Now()}
